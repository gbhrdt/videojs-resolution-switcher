...
(function () {
	...
	(function (window, videojs) {
        ...
        var btn = this.el();
        var btnInner = btn.getElementsByTagName('button');
        if (btnInner && btnInner.length) {
          btn = btnInner[0];
        }

				if (options.dynamicLabel) {
					videojs.dom.addClass(this.label, 'vjs-resolution-button-label');
					btn.appendChild(this.label);
				} else {
		...
		ResolutionMenuButton.prototype.updateLabel = function () {
			this.label.innerHTML = this.player_.currentResolutionState.label;
		};
        ...
        player.one(handleSeekEvent, handleLoad);
				function handleLoad() {
